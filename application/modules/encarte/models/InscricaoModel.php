...
class InscricaoModel extends Crud {

    private $table = 'enc_inscricao';
    private $primary_key = 'cod_inscricao';
    ...
    private function atualizar($var) {
        ...
        $criterios = Array(
            $this->primary_key => $fields['cod_inscricao']
        );
    ...
    public function salvarInscricaoCategoria($id, $campos) {

        //$var = array_push($campos,$id);
        $fields['cod_inscricao'] = $id;
        foreach ($campos as $categoria) {
            $fields['cod_categoria'] = $categoria;
            $filter = Array(
                'table' => 'enc_inscricao_categoria',
                'fields' => $fields
            );
            $dados = $this->insert($filter);
        }
